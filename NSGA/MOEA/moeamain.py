...
"""
MOEA/D (Batch-level encoding) — drop-in comparable baseline for your NSGA-II codebase
...
- HARD penalties only for:
    miss_alloc, miss_tt, cap_excess
- Same encoding (batch-level shares) + same operators:
    structural crossover + adaptive roulette mutation (add/del/mod/mode)
- Same analytics outputs:
    HV, IGD+ (P*), Spacing, feasible ratio, roulette plots, Pareto 3D plots, runtime Excel, result.txt

Key difference:
- Environmental selection is replaced by MOEA/D neighbourhood replacement based on scalar decomposition.
"""
...
# HARD_TIME_WINDOW=False: lateness allowed but costed (Scheme A)
# HARD_TIME_WINDOW=True : lateness forbidden (hard infeasible)
...
# Waiting in objectives
WAITING_COST_PER_TEU_HOUR_DEFAULT = 0.5
WAIT_EMISSION_gCO2_per_TEU_H_DEFAULT = 0.0

# Lateness in cost (Scheme A)
LATE_COST_PER_TEU_HOUR = 1e6

# Static roulette priors (only as initial priors; adaptive roulette will evolve)
W_ADD = 0.25
W_DEL = 0.20
W_MOD = 0.35
W_MODE = 0.20
...
# ========================
# Dominance (for logging + final Pareto)
# ========================
...
# ========================
# Metrics: HV (Monte Carlo), P*, IGD+, Spacing
# ========================
...
def choose_simplex_H_for_pop(pop_size: int, m: int = 3, H_max: int = 50) -> Tuple[int, int]:
    # For m=3: N = (H+1)(H+2)/2
    best_H = 1
    best_N = (best_H + 1) * (best_H + 2) // 2
    best_gap = abs(best_N - pop_size)
...
def generate_weights_simplex_lattice(H: int, m: int = 3) -> np.ndarray:
    if m != 3:
        raise ValueError("This implementation currently supports m=3.")
    w = []
    for i in range(H + 1):
    ...
    # avoid zeros all around; but lattice already fine. Keep.
    return W


def build_weight_vectors(pop_size: int, m: int = 3, seed: int = 0) -> np.ndarray:
    rng = np.random.default_rng(seed)
    H, N = choose_simplex_H_for_pop(pop_size, m=m)
    W = generate_weights_simplex_lattice(H, m=m)  # shape (N, m)
    if len(W) == pop_size:
        return W
    if len(W) > pop_size:
        idx = rng.choice(len(W), size=pop_size, replace=False)
        return W[idx]
    # len(W) < pop_size => pad by random weights on simplex
    extra = pop_size - len(W)
    r = rng.random((extra, m))
    r = r / np.sum(r, axis=1, keepdims=True)
    return np.vstack([W, r])


def build_neighborhood(W: np.ndarray, T: int) -> List[List[int]]:
    N = W.shape[0]
    neigh = []
    for i in range(N):
        d = np.linalg.norm(W - W[i], axis=1)
        idx = np.argsort(d)[:max(1, min(T, N))].tolist()
        neigh.append(idx)
    return neigh


def normalize_obj(obj: Tuple[float, float, float], z_min: np.ndarray, z_max: np.ndarray, eps: float = 1e-12) -> np.ndarray:
    o = np.array(obj, dtype=float)
    denom = (z_max - z_min)
    denom = np.where(np.abs(denom) < eps, 1.0, denom)
    return (o - z_min) / denom


def scalar_tchebycheff(obj_norm: np.ndarray, w: np.ndarray) -> float:
    # g = max_m w_m * |f_m - 0| since we normalize relative to (z_min,z_max) -> ideal is 0
    # Use a tiny epsilon to avoid zero weight nullifying a dimension
    ww = np.maximum(w, 1e-6)
    return float(np.max(ww * np.abs(obj_norm)))
...
def moead_better(
    ...
    # constraint-aware replacement
    if cand.feasible and (not cur.feasible):
        return True
    if (not cand.feasible) and cur.feasible:
        return False

    if (not cand.feasible) and (not cur.feasible):
        if cand.penalty < cur.penalty - 1e-12:
            return True
        if cur.penalty < cand.penalty - 1e-12:
            return False
        # tie -> compare scalar
        cn = normalize_obj(cand.objectives, z_min, z_max)
        un = normalize_obj(cur.objectives, z_min, z_max)
        return scalar_tchebycheff(cn, w) < scalar_tchebycheff(un, w) - 1e-12

    # both feasible -> compare scalar decomposition
    cn = normalize_obj(cand.objectives, z_min, z_max)
    un = normalize_obj(cur.objectives, z_min, z_max)
    return scalar_tchebycheff(cn, w) < scalar_tchebycheff(un, w) - 1e-12
...
# ========================
# Runner (one run) — MOEA/D
# ========================

def run_moead_analytics(
    filename="data.xlsx",
    pop_size=60,
    generations=200,
    neigh_T: int = 10,
    delta_neigh_parent: float = 0.9,
    nr_replace: int = 2,
    p_crossover: float = 0.7,
    p_mutation: float = 0.35,
    seed_for_weights: int = 0,
):
    print("Loading data...")
    ...
    # weight vectors & neighborhood
    W = build_weight_vectors(pop_size=pop_size, m=3, seed=seed_for_weights)  # (N,3)
    N = W.shape[0]
    B = build_neighborhood(W, T=neigh_T)
    ...
    # HV ref point per run (same style as your NSGA-II code)
    ...
    print(f"[MOEA/D] N={N}, neigh_T={neigh_T}, delta={delta_neigh_parent}, nr={nr_replace}")

    # z_min/z_max for normalization (use current population; update online)
    # Use feasible if exists, else use all
    feas = [ind for ind in population if ind.feasible]
    base = feas if feas else population
    z_min = np.min(np.array([i.objectives for i in base], dtype=float), axis=0)
    z_max = np.max(np.array([i.objectives for i in base], dtype=float), axis=0)

    # history (same containers as NSGA-II version)
    ...
    for gen in range(generations):
        ...
        # ===== MOEA/D main loop: for each subproblem i produce one offspring and update neighborhood
        for i in range(N):
            # parent pool
            if random.random() < delta_neigh_parent:
                pool_idx = B[i]
            else:
            ...
            p1_idx, p2_idx = random.sample(pool_idx, 2)
            p1, p2 = population[p1_idx], population[p2_idx]

            # crossover (produce two children, we take one)
            if random.random() < p_crossover:
                c1, _ = crossover_structural(p1, p2, batches)
            else:
            ...
            # mutation (same style as NSGA-II)
            ...
            # update z_min/z_max (prefer feasible if any exist)
            # rule: if we already have feasible solutions in population, update mins/maxs using feasible only
            pop_feas = [x for x in population if x.feasible]
            if pop_feas:
                cand_set = pop_feas + ([c1] if c1.feasible else [])
            else:
                cand_set = population + [c1]

            objs_arr = np.array([x.objectives for x in cand_set], dtype=float)
            z_min = np.minimum(z_min, np.min(objs_arr, axis=0))
            z_max = np.maximum(z_max, np.max(objs_arr, axis=0))

            # neighborhood replacement
            replaced = 0
            neigh_idx = B[i]
            random.shuffle(neigh_idx)

            for j in neigh_idx:
                if moead_better(c1, population[j], W[j], z_min, z_max):
                    population[j] = deepcopy(c1)
                    replaced += 1
                    if replaced >= max(1, nr_replace):
                        break
...
def save_pareto_solutions(pareto: List[Individual], batches: List[Batch], filename: str = "result.txt"):
    ...
    with open(filename, "w", encoding="utf-8") as f:
        f.write("===== MOEA/D Pareto Solutions (Feasible Only) =====\n\n")
        if not pareto:
...
if __name__ == "__main__":
    ...
    # MOEA/D params
    neigh_T = 10          # neighbourhood size
    delta = 0.9           # parent selection from neighbourhood prob
    ...
    print(f"[CONFIG] Algorithm=MOEA/D")
    print(f"[CONFIG] HARD_TIME_WINDOW={HARD_TIME_WINDOW}  (False=lateness soft via cost; True=lateness hard)")
    print(f"[CONFIG] Waiting defaults: cost={WAITING_COST_PER_TEU_HOUR_DEFAULT}, emis_g={WAIT_EMISSION_gCO2_per_TEU_H_DEFAULT}")
    print(f"[CONFIG] Lateness in cost: LATE_COST_PER_TEU_HOUR={LATE_COST_PER_TEU_HOUR:.3e}")
    print(f"[CONFIG] MOEA/D: neigh_T={neigh_T}, delta={delta}, nr={nr}, pX={p_crossover}, pM={p_mutation}")
    ...
    for run_id in range(runs):
        ...
        pop, pareto, batches, front_hist, hv_hist, fr_hist, vio_hist, mut_tracker = run_moead_analytics(
            filename=filename,
            pop_size=pop_size,
            generations=generations,
            neigh_T=neigh_T,
            delta_neigh_parent=delta,
            nr_replace=nr,
            p_crossover=p_crossover,
            p_mutation=p_mutation,
            seed_for_weights=seed,  # tie weights to run seed for reproducibility
        )
    ...
    excel_name = "run_time_summary_moead.xlsx"
    ...
    if best_pareto is not None and best_batches is not None:
        ...
        save_pareto_solutions(best_pareto, best_batches, filename="result_moead.txt")
    ...
    plot_pareto_3d_final_only(
        ...
        save="pareto_3d_best_run_final_moead.png",
        title=f"Pareto Front (Final Gen) - Best MOEA/D Run #{best_run_idx}"
    )
    ...
    plot_pareto_3d_all_generations(
        ...
        save="pareto_3d_allgens_best_run_moead.png",
        title=f"Pareto Points Across {generations} Generations (Best MOEA/D Run #{best_run_idx})",
        cmap_name="turbo"
    )

    # ===== Academic plots (metrics & mutation)
    plot_mutation_attempt_stacked(gen, mut_agg["share_mean"], save="mutation_attempt_stacked_moead.png")
    plot_mutation_success_rate_2x2(gen, mut_agg["rate_mean"], mut_agg["rate_std"], save="mutation_success_rate_moead.png")
    plot_mutation_effective_contribution(gen, mut_agg["eff_mean"], save="mutation_effective_contribution_moead.png")
    if "prob_mean" in mut_agg:
        plot_mutation_adaptive_prob_stacked(gen, mut_agg["prob_mean"], save="mutation_adaptive_prob_stacked_moead.png")

    plot_convergence_curves(gen, hv_mean, hv_std, igd_mean, igd_std, save="convergence_hv_igd_moead.png")
    plot_spacing_curve(gen, sp_mean, sp_std, save="diversity_spacing_moead.png")
    plot_feasible_ratio(gen, fr_mean, fr_std, save="feasible_ratio_moead.png")
    plot_violation_breakdown_stacked(gen, vio_mean_dict_mean, save="violation_breakdown_moead.png")

    print("\nSaved figures:")
    print(" - pareto_3d_best_run_final_moead.png")
    print(" - pareto_3d_allgens_best_run_moead.png")
    print(" - mutation_attempt_stacked_moead.png")
    print(" - mutation_success_rate_moead.png")
    print(" - mutation_effective_contribution_moead.png")
    print(" - mutation_adaptive_prob_stacked_moead.png (if available)")
    print(" - convergence_hv_igd_moead.png")
    print(" - diversity_spacing_moead.png")
    print(" - feasible_ratio_moead.png")
    print(" - violation_breakdown_moead.png")
    print(" - run_time_summary_moead.xlsx")
    print(" - result_moead.txt")
