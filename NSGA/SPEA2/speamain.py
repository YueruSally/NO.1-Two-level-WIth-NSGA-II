...
"""
SPEA2 (Batch-level encoding) — based on your current NSGA-II codebase

What stays the SAME (fair comparison):
- Representation: batch-level gene key=(origin,destination,batch_id) -> list[PathAllocation(path, share)]
- Evaluation: Scheme A objectives (cost incl. waiting + lateness, emission incl. waiting, makespan),
  plus HARD penalties (miss_alloc, miss_tt, cap_excess) and feasible flags.
- Operators: structural crossover + 4 mutation operators (add/del/mod/mode) with Adaptive Roulette.
- Plots & analytics: HV (MC), IGD+ with P*, Spacing, Feasible ratio, mutation plots, Pareto 3D plots, runtime Excel.

What changes (SPEA2 core):
- Replace NSGA-II environmental selection with SPEA2 fitness:
    F(i) = R(i) + D(i),
    R(i) = sum_{j dominates i} S(j),  S(j) = #{k | j dominates k}
    D(i) = 1 / (sigma_k(i) + 2)  (kNN density in normalised objective space)
- External archive selection with truncation to keep archive size = pop_size.

Dominance used inside SPEA2 uses your existing constraint-domination (dominates()) so that
constraint handling remains consistent across algorithms.

Reference:
- Zitzler, Laumanns, Thiele (2001), SPEA2. (You can cite this in your paper.)
"""
...
# Lateness cost per TEU-hour late (Scheme A)
...
# ========================
# Dominance (constraint-domination)
# ========================
...
def _normalise_objectives(objs: np.ndarray) -> np.ndarray:
    """
    Min-max normalisation for distance / density in objective space.
    objs: (N,3)
    """
...
def spea2_fitness(combined: List[Individual]) -> Tuple[Dict[Individual, float], Dict[Individual, float], Dict[Individual, float]]:
    """
    Returns:
      F: total fitness (smaller is better)
      R: raw fitness
      D: density
    Uses your dominance() (constraint-domination).
    Density uses kNN distance in normalised objective space.
    """
    N = len(combined)
    if N == 0:
        return {}, {}, {}
    ...
    # Raw fitness R(i): sum of strengths of solutions dominating i
    R = {ind: 0.0 for ind in combined}
    for i in range(N):
        pi = combined[i]
        r = 0.0
        for j in range(N):
    ...
    objs = np.array([ind.objectives for ind in combined], dtype=float)
    objs_n = _normalise_objectives(objs)

    # distances matrix
    dist = np.sqrt(np.sum((objs_n[:, None, :] - objs_n[None, :, :]) ** 2, axis=2))
    ...
    for i, ind in enumerate(combined):
        kth = np.partition(dist[i], k - 1)[k - 1]  # k-th nearest (1-indexed) in 0-index
        D[ind] = 1.0 / (float(kth) + 2.0)

    # Total fitness
    F = {ind: float(R[ind] + D[ind]) for ind in combined}
    return F, R, D


def spea2_truncate_archive(archive: List[Individual], target_size: int) -> List[Individual]:
    """
    SPEA2 truncation: repeatedly remove the individual located in the most crowded region.
    Uses distances in normalised objective space.
    """
    if len(archive) <= target_size:
        return archive

    objs = np.array([ind.objectives for ind in archive], dtype=float)
    objs_n = _normalise_objectives(objs)

    dist = np.sqrt(np.sum((objs_n[:, None, :] - objs_n[None, :, :]) ** 2, axis=2))
    np.fill_diagonal(dist, np.inf)

    keep = list(range(len(archive)))

    while len(keep) > target_size:
        # For each candidate, build sorted neighbor distances (lexicographic rule)
        best_idx = None
        best_profile = None

        for idx in keep:
            drow = dist[idx, keep]
            dsorted = np.sort(drow)  # ascending: nearest first
            profile = tuple(dsorted.tolist())

            if best_profile is None or profile < best_profile:
                best_profile = profile
                best_idx = idx

        keep.remove(best_idx)

    return [archive[i] for i in keep]


def spea2_environmental_selection(combined: List[Individual], archive_size: int) -> Tuple[List[Individual], Dict[Individual, float], Dict[Individual, float], Dict[Individual, float]]:
    """
    Standard SPEA2 environmental selection:
    - compute fitness
    - archive A = {i | F(i) < 1}
    - if |A| < archive_size: add best remaining by F
    - if |A| > archive_size: truncate
    """
    F, R, D = spea2_fitness(combined)

    archive = [ind for ind in combined if F[ind] < 1.0]

    if len(archive) < archive_size:
        remaining = [ind for ind in combined if ind not in archive]
        remaining.sort(key=lambda x: F[x])
        archive.extend(remaining[: max(0, archive_size - len(archive))])

    if len(archive) > archive_size:
        archive = spea2_truncate_archive(archive, archive_size)

    return archive, F, R, D


def spea2_binary_tournament(pop: List[Individual], F: Dict[Individual, float], D: Dict[Individual, float]) -> Individual:
    """
    Parent selection for SPEA2:
    Prefer smaller F; tie-break by smaller D (less crowded -> smaller D? Actually D is larger when crowded,
    so smaller D means less crowded).
    """
    ...
    if fb < fa:
        return b
    # tie: choose less crowded (smaller density term D)
    da, db = D[a], D[b]
...
# ========================
# Plot helpers (mutation + metrics) (unchanged)
# ========================
...
# ========================
# Hypervolume (unchanged)
# ========================

class HypervolumeCalculator:
    """
    Monte Carlo HV in 3D.
    Keep reference point fixed within each run.
    """
...
# ========================
# Runner: SPEA2 (one run)
# ========================

def run_spea2_analytics(filename="data.xlsx", pop_size=60, generations=200):
    print("Loading data...")
    ...
    # SPEA2 initial environmental selection to make population an archive
    combined0 = population[:]  # no offspring yet
    population, F_fit, R_fit, D_fit = spea2_environmental_selection(combined0, archive_size=pop_size)

    for gen in range(generations):
        for op in OPS:
            mut_tracker["prob"][op][gen] = float(roulette.prob[op])

        # metrics front0 from current population (archive)
        ...
        if gen % 10 == 0 or gen == generations - 1:
            ...
            best_F = min(F_fit[ind] for ind in population) if population else float("inf")
            best_cost = min(ind.objectives[0] for ind in front0_unique) if front0_unique else float("inf")
            print(f"Gen {gen:03d} | Pop={len(population):2d} | Front0={len(front0):2d} | FeasFront0={feas_n:2d} | "
                  f"FeasRatio={feasible_ratio_hist[-1]:.2%} | BestCost={best_cost:.3e} | BestFitF={best_F:.3e} | HV={hv_history[-1]:.4f}")

        # ========= SPEA2 mating pool (binary tournament based on fitness)
        mating_pool: List[Individual] = []
        while len(mating_pool) < pop_size:
            mating_pool.append(spea2_binary_tournament(population, F_fit, D_fit))

        # ========= offspring generation (same operators)
        ...
        population, F_fit, R_fit, D_fit = spea2_environmental_selection(combined, archive_size=pop_size)

    # final pareto: feasible-only, from final archive
    final_fronts = non_dominated_sort(population)
    f0 = final_fronts[0] if final_fronts else []
...
def save_pareto_solutions(pareto: List[Individual], batches: List[Batch], filename: str = "result.txt"):
    ...
    with open(filename, "w", encoding="utf-8") as f:
        f.write("===== SPEA2 Pareto Solutions (Feasible Only) =====\n\n")
        if not pareto:
...
if __name__ == "__main__":
    ...
    print(f"[CONFIG] Algorithm=SPEA2 (with your constraint-domination + adaptive roulette)")
    print(f"[CONFIG] HARD_TIME_WINDOW={HARD_TIME_WINDOW}  (False=lateness soft via cost; True=lateness hard)")
    print(f"[CONFIG] Waiting in objectives: default cost_per_teu_h={WAITING_COST_PER_TEU_HOUR_DEFAULT}, default emis_g_per_teu_h={WAIT_EMISSION_gCO2_per_TEU_H_DEFAULT}")
    ...
    for run_id in range(runs):
        ...
        pop, pareto, batches, front_hist, hv_hist, fr_hist, vio_hist, mut_tracker = run_spea2_analytics(
    ...
    excel_name = "run_time_summary_spea2.xlsx"
    ...
    hv_runs = np.array(run_hv_hist, dtype=float)
    ...
    if best_pareto is not None and best_batches is not None:
        ...
        save_pareto_solutions(best_pareto, best_batches, filename="result_spea2.txt")
    ...
    plot_pareto_3d_final_only(
        ...
        save="pareto_3d_best_run_final_spea2.png",
        title=f"Pareto Front (Final Gen) - SPEA2 Best Run #{best_run_idx}"
    )
    ...
    plot_pareto_3d_all_generations(
        ...
        save="pareto_3d_allgens_best_run_spea2.png",
        title=f"Pareto Points Across {generations} Generations (SPEA2 Best Run #{best_run_idx})",
        cmap_name="turbo"
    )

    # ===== Academic plots (metrics & mutation)
    plot_mutation_attempt_stacked(gen, mut_agg["share_mean"], save="mutation_attempt_stacked_spea2.png")
    plot_mutation_success_rate_2x2(gen, mut_agg["rate_mean"], mut_agg["rate_std"], save="mutation_success_rate_spea2.png")
    plot_mutation_effective_contribution(gen, mut_agg["eff_mean"], save="mutation_effective_contribution_spea2.png")
    if "prob_mean" in mut_agg:
        plot_mutation_adaptive_prob_stacked(gen, mut_agg["prob_mean"], save="mutation_adaptive_prob_stacked_spea2.png")

    plot_convergence_curves(gen, hv_mean, hv_std, igd_mean, igd_std, save="convergence_hv_igd_spea2.png")
    plot_spacing_curve(gen, sp_mean, sp_std, save="diversity_spacing_spea2.png")
    plot_feasible_ratio(gen, fr_mean, fr_std, save="feasible_ratio_spea2.png")
    plot_violation_breakdown_stacked(gen, vio_mean_dict_mean, save="violation_breakdown_spea2.png")

    print("\nSaved figures:")
    print(" - pareto_3d_best_run_final_spea2.png")
    print(" - pareto_3d_allgens_best_run_spea2.png")
    print(" - mutation_attempt_stacked_spea2.png")
    print(" - mutation_success_rate_spea2.png")
    print(" - mutation_effective_contribution_spea2.png")
    print(" - mutation_adaptive_prob_stacked_spea2.png (if available)")
    print(" - convergence_hv_igd_spea2.png")
    print(" - diversity_spacing_spea2.png")
    print(" - feasible_ratio_spea2.png")
    print(" - violation_breakdown_spea2.png")
    print(" - run_time_summary_spea2.xlsx")
    print(" - result_spea2.txt")
