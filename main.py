...
# ========================
# NEW: Analytics speed knobs (recommended default: fast + stable)
# ========================
HV_EVERY = 5               # compute HV every HV_EVERY generations
HV_SAMPLES = 2000          # HV Monte Carlo samples (1000-3000 typical)
...
# --- MODIFIED to match Program B:
# Program B uses ONLY the first timetable entry entries[0], NOT the minimum over all entries.
...
class HypervolumeCalculator:
    def __init__(self, ref_point: Tuple[float, float, float], num_samples=2000):
...
def run_nsga2_analytics(filename="data.xlsx", pop_size=60, generations=200):
    ...
    hv_calc = HypervolumeCalculator(ref_point=tuple(ref_point), num_samples=HV_SAMPLES)
    print(f"Reference Point for HV: {ref_point} | HV_SAMPLES={HV_SAMPLES} | HV_EVERY={HV_EVERY}")
    ...
    for gen in range(generations):
        ...
        if gen % 10 == 0 or gen == generations - 1:
            ...
            print(f"Gen {gen:03d} | Front0={len(front0):2d} | FeasFront0={feas_n:2d} | "
                  ...
                  f"BestCost={best_cost:.3e} | BestPenalty={best_pen:.3e} | HV={hv_history[-1]:.4f}")
...
# ========================
# NEW: Export Pareto solutions to JSON (for compare)
# ========================
import json
    ...
    for ind in pareto:
        sol = {
            "objectives": {
                ...
                "makespan_h": float(ind.objectives[2]),
...
if __name__ == "__main__":
    ...
    for run_id in range(runs):
        ...
        pop, pareto, batches, front_hist, hv_hist, fr_hist, fr_strict_hist, vio_hist, mut_tracker = run_nsga2_analytics(
        ...
        final_fr_strict = fr_strict_hist[-1] if fr_strict_hist else 0.0
    ...
    print(f"FeasRatio (strict no-late)| best={frs_best:.2%}, worst={frs_worst:.2%}, mean={frs_m:.2%}, std={frs_s:.2%}")
