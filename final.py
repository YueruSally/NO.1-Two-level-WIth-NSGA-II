...
TIME_BUCKET_H = 1.0  # simulation time granularity: 1 hour
CHINA_REGIONS = {"CN", "China"}
EUROPE_REGIONS = {"WE", "EE", "EU", "Europe"}

BIG_M = 1e6                 # large constant for capacity penalty
INFEASIBLE_PENALTY = BIG_M * 1e6  # hard penalty to eliminate infeasible solutions
...
class Path:
    ...
    # Ignore ID; compare structure only
...
class Individual:
    ...
    # Objectives: Cost, Emission, Time (with penalties included)
...
def load_network_from_extended(filename: str):
    ...
    DAILY_HOURS = 24.0  # capacity in excel assumed TEU/day; convert to TEU/timebucket
...
def random_dfs_paths(graph, origin, dest, max_len=8, max_paths=120) -> List[List[Arc]]:
    """
    Randomized DFS enumeration (bounded).
    NOTE: increased max_paths to improve diversity.
    """
    paths: List[List[Arc]] = []
...
def build_path_library(node_names, arcs, batches, timetables) -> Dict[Tuple[str, str], List[Path]]:
    ...
    for batch in batches:
        ...
        if paths_for_od:
            ...
            K_emis = 20
            K_rand = 40
            K_total = K_cost + K_time + K_emis + K_rand

            by_cost = sorted(paths_for_od, key=lambda p: p.base_cost_per_teu)[:K_cost]
            by_time = sorted(paths_for_od, key=lambda p: p.base_travel_time_h)[:K_time]
            by_emis = sorted(paths_for_od, key=lambda p: p.base_emission_per_teu)[:K_emis]

            pool = list({p: 1 for p in (by_cost + by_time + by_emis)}.keys())
            remaining = [p for p in paths_for_od if p not in pool]
            random.shuffle(remaining)
            pool.extend(remaining[:K_rand])

            path_lib[od] = pool[:K_total]
...
def simulate_path_time_capacity(path: Path, batch: Batch, flow: float, tt_dict,
                               arc_flow_map) -> float:
    """
    Simulate traversal time and capacity usage along a path.
    - Road: depart immediately
    - Rail/Water: wait for next timetable departure
    """
    t = batch.ET
...
def mutate_mode_two_arcs(ind: Individual,
                         ...
                         p: float = 1.0):
    """
    Scheme A: choose one batch and one path in it, then randomly choose up to 2 arc positions
    that have parallel arcs (same (u,v) but different mode) and swap the arc + mode.
    """
    if random.random() > p:
        return
    if not batches:
...
class HypervolumeCalculator:
    ...
    def calculate(self, pareto_front_inds: List[Individual]) -> float:
        ...
        denom = np.where(self.ref_point <= 0, 1.0, self.ref_point)
        norm_objs = front_objs / denom
        norm_objs = np.clip(norm_objs, 0.0, 1.0)

        valid_mask = np.all(norm_objs <= 1.0, axis=1)
        valid_objs = norm_objs[valid_mask]
        if len(valid_objs) == 0:
            return 0.0
...
def run_nsga2_analytics(filename="data.xlsx", pop_size=50, generations=100):
    print("Loading data...")
    ...
    print(f"Reference Point for HV (real scale) set to: {ref_point}")
...
def print_pure_structure(ind: Individual, batches: List[Batch], sol_name="Solution"):
    print(f"\n===== {sol_name} structure (Node+Mode | Share) =====")
    for batch in batches:
        key = (batch.origin, batch.destination, batch.batch_id)
        allocs = ind.od_allocations.get(key, [])
        if not allocs:
...
def save_pareto_solutions(pareto: List[Individual],
    ...
    with open(filename, "w", encoding="utf-8") as f:
        f.write("===== NSGA-II Pareto Solutions (Unique) =====\n\n")
...
def plot_results(history_fronts, hv_history):
    ...
    ax2.plot(range(len(hv_history)), hv_history, marker='o', linestyle='-', markersize=4)
    ax2.set_xlabel('Generation')
    ax2.set_ylabel('Hypervolume (0–1, Normalised)')
...
# ========================
# 9. Typical solution visualisations
# ========================
...
def select_weighted_solution(pareto: List[Individual], weights=(0.4, 0.3, 0.3)) -> Tuple[Individual, float]:
    norm_objs, _, _ = normalise_objectives(pareto)
    ...
    for batch in batches:
        ...
        for a in allocs:
            ...
            for arc in a.path.arcs:
                mode_flow[arc.mode] = mode_flow.get(arc.mode, 0.0) + flow * arc.distance
    return mode_flow


def plot_mode_share(ind: Individual, batches: List[Batch], filename: str = "typical_solution_mode_share.png"):
...
def plot_radar_for_solution(ind: Individual, pareto: List[Individual], filename: str = "typical_solution_radar.png"):
    norm_objs, _, _ = normalise_objectives(pareto)
...
if __name__ == "__main__":
    ...
    for run_id in range(runs):
        ...
        final_hv = h_hv[-1] if h_hv else 0.0
    ...
    hv_mean = float(np.mean(all_final_hv)) if all_final_hv else 0.0
    hv_std = float(np.std(all_final_hv)) if all_final_hv else 0.0
    ...
    if best_pareto is not None and best_batches is not None:
        ...
        typical_ind, typical_score = select_weighted_solution(best_pareto, weights=(0.4, 0.3, 0.3))
        ...
        plot_mode_share(typical_ind, best_batches, filename="typical_solution_mode_share.png")
        plot_radar_for_solution(typical_ind, best_pareto, filename="typical_solution_radar.png")